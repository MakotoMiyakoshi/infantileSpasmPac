...
%% Separate variance reduction figures into Awake and Sleep.
clear all
load /data/mobi/Hiroki/p4200_select50subjects/summaryData
load /data/mobi/Hiroki/p4220_calculateVarianceChange/varRatioTensor

% Obtain sleepAwakeIdx.
awakeIdx = find(pacGroupIdx==1 | pacGroupIdx==3);
sleepIdx = find(pacGroupIdx==2 | pacGroupIdx==4);

% Load one dummy dataset.
allSets = dir('/data/mobi/Hiroki/p4000_import_preprocess_n600/*.set');
loadName = allSets(1).name;
...
%%% Awake %%%
%%%%%%%%%%%%%
asr_Raw = (1-mean(varRatioTensor(:,1,awakeIdx),3))*-100;
ic1_Raw = (1-mean(varRatioTensor(:,2,awakeIdx),3))*-100;
ic2_Raw = (1-mean(varRatioTensor(:,3,awakeIdx),3))*-100;
ic1_asr = (1-mean(varRatioTensor(:,4,awakeIdx),3))*-100;
ic2_asr = (1-mean(varRatioTensor(:,5,awakeIdx),3))*-100;
ic2_ic1 = (1-mean(varRatioTensor(:,6,awakeIdx),3))*-100;

figure
subplot(3,3,1)
topoplot(asr_Raw, EEG.chanlocs, 'maplimits', [-80 0]); colormap('winter')
title(sprintf('var. reduc.\n from %.0f to %.0f%%', max(asr_Raw), min(asr_Raw)))

subplot(3,3,2)
topoplot(ic1_Raw, EEG.chanlocs, 'maplimits', [-80 0]); colormap('winter')
title(sprintf('var. reduc.\n from %.0f to %.0f%%', max(ic1_Raw), min(ic1_Raw)))

subplot(3,3,3)
topoplot(ic2_Raw, EEG.chanlocs, 'maplimits', [-80 0]); colormap('winter')
title(sprintf('var. reduc.\n from %.0f to %.0f%%', max(ic2_Raw), min(ic2_Raw)))

subplot(3,3,5)
topoplot(ic1_asr, EEG.chanlocs, 'maplimits', [-80 0]); colormap('winter')
title(sprintf('var. reduc.\n from %.0f to %.0f%%', max(ic1_asr), min(ic1_asr)))

subplot(3,3,6)
topoplot(ic2_asr, EEG.chanlocs, 'maplimits', [-80 0]); colormap('winter')
title(sprintf('var. reduc.\n from %.0f to %.0f%%', max(ic2_asr), min(ic2_asr)))

subplot(3,3,9)
topoplot(ic2_ic1, EEG.chanlocs, 'maplimits', [-80 0]); colormap('winter')
title(sprintf('var. reduc.\n from %.0f to %.0f%%', max(ic2_ic1), min(ic2_ic1)))
...
print('/data/mobi/Hiroki/p4220_calculateVarianceChange/varChangesAwake', '-dsvg')
...
%%% Sleep %%%
%%%%%%%%%%%%%
asr_Raw = (1-mean(varRatioTensor(:,1,sleepIdx),3))*-100;
ic1_Raw = (1-mean(varRatioTensor(:,2,sleepIdx),3))*-100;
ic2_Raw = (1-mean(varRatioTensor(:,3,sleepIdx),3))*-100;
ic1_asr = (1-mean(varRatioTensor(:,4,sleepIdx),3))*-100;
ic2_asr = (1-mean(varRatioTensor(:,5,sleepIdx),3))*-100;
ic2_ic1 = (1-mean(varRatioTensor(:,6,sleepIdx),3))*-100;
...
% Print.
set(gcf, 'position', [1           2        1858         929])
print('/data/mobi/Hiroki/p4220_calculateVarianceChange/varChangesSleep', '-dsvg')



% asr_Raw = (1-mean(varRatioTensor(:,1,:),3))*-100;
% ic1_Raw = (1-mean(varRatioTensor(:,2,:),3))*-100;
% ic2_Raw = (1-mean(varRatioTensor(:,3,:),3))*-100;
% ic1_asr = (1-mean(varRatioTensor(:,4,:),3))*-100;
% ic2_asr = (1-mean(varRatioTensor(:,5,:),3))*-100;
% ic2_ic1 = (1-mean(varRatioTensor(:,6,:),3))*-100;
% 
...
% figure
% subplot(3,3,1)
% topoplot(zscore(asr_Raw), EEG.chanlocs, 'maplimits', [-3 3]); colormap('winter')
% title(sprintf('var. reduc.\n%.0f-%.0f%% (SD %.0f)', abs(max(asr_Raw)*100), abs(min(asr_Raw)*100), mean(asr_Raw_std)*100))
% 
% subplot(3,3,2)
% topoplot(zscore(ic1_Raw), EEG.chanlocs, 'maplimits', [-3 3]); colormap('winter')
% title(sprintf('var. reduc.\n%.0f-%.0f%% (SD %.0f)', abs(max(ic1_Raw)*100), abs(min(ic1_Raw)*100), mean(ic1_Raw_std)*100))
% 
% subplot(3,3,3)
% topoplot(zscore(ic2_Raw), EEG.chanlocs, 'maplimits', [-3 3]); colormap('winter')
% title(sprintf('var. reduc.\n%.0f-%.0f%% (SD %.0f)', abs(max(ic2_Raw)*100), abs(min(ic2_Raw)*100), mean(ic2_Raw_std)*100))
% 
% subplot(3,3,5)
% topoplot(zscore(ic1_asr), EEG.chanlocs, 'maplimits', [-3 3]); colormap('winter')
% title(sprintf('var. reduc.\n%.0f-%.0f%% (SD %.0f)', abs(max(ic1_asr)*100), abs(min(ic1_asr)*100), mean(ic1_asr_std)*100))
% 
% subplot(3,3,6)
% topoplot(zscore(ic2_asr), EEG.chanlocs, 'maplimits', [-3 3]); colormap('winter')
% title(sprintf('var. reduc.\n%.0f-%.0f%% (SD %.0f)', abs(max(ic2_asr)*100), abs(min(ic2_asr)*100), mean(ic2_asr_std)*100))
% 
% subplot(3,3,9)
% topoplot(zscore(ic2_ic1), EEG.chanlocs, 'maplimits', [-3 3]); colormap('winter')
